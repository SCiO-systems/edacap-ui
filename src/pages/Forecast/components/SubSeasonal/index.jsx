...
import { Map, CurrentData, HistoricalData, LayerPanel, BarChart, PointBarChart, DatePicker } from './components';
import StationsService from '../../../../services/httpService/stationsService';
import '../../../../utils/singleTileSubSeasonal';
import './styles.css';
...
const Seasonal = () => {
	...
	const setSubSeasonalLayersArray = (payload) => dispatch({ type: Actions.SetSubSeasonalLayersArray, payload });
	...
	const [time, setTime] = useState({ year: '', month: '', week: '' });

	useEffect(
		() => {
			if (!map) return;
			if (time.year === '' || time.month === '' || time.week === '') return;
			const temp = tabLayers.find((item) => item.category === 'seasonal');
			if (temp && locationLayer) {
				const name = 'aclimate_et:subseasonal_country_et_dominant';
				const index = 0;
				...
				const wmsLayer = L.singleTileSubSeasonal(`${temp.geoserver_domain}/${temp.workspace}/wms`, [[x1, x2], [y1, y2]], {
					...
					zIndex: index + 1,
					opacity: 1,
					time: `${time.year}-${time.month}-${time.week}`,
				});
				const layer = {
					layer: wmsLayer,
					configuration: {
				...
				setSubSeasonalLayersArray([layer]);
			}
